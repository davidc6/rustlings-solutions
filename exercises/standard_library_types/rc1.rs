...
fn main() {
    ...
    let mercury = Planet::Mercury(Rc::clone(&sun)); // does not actually clone the value but creates a copy of the pointer
    ...
    drop(neptune); // decrements the strong reference count
    println!("reference count = {}", Rc::strong_count(&sun)); // 8 references
